...
function MyApp({ Component, pageProps }: AppProps) {
  const { data: stationData, error } = useSWR<HelloData>("/api/hello", fetcher, {
    refreshInterval: (data) => {
      if (data && data.next) {
        const nextEpisodeStart = (Number(data.next.episode_timestamps.episode_start) * 1000)
        const currentTime = (+ new Date())
        if (currentTime >= nextEpisodeStart) {
          console.info('possibly fetched outdated info, retrying in a moment...')
          return 5000
        }
        const millisecondsToNextShow = nextEpisodeStart - currentTime + 3000
        console.info(`fetching data again in ${millisecondsToNextShow / 1000} seconds...`)
